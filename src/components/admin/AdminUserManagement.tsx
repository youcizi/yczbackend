...
export const AdminUserManagement: React.FC<UserListProps> = ({ activePlugins = [] }) => {
  ...
  const handleAdjustSubmit = async (e: React.FormEvent) => { e.preventDefault(); setIsLoading(true); try { const res = await fetch(`/api/v1/users/${adjustType}/adjust`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(adjustData) }); if ((await res.json()).success) { setIsAdjustOpen(false); fetchLogsAndTokens(); fetchData(); } } catch (e) { setError('调整失败'); } finally { setIsLoading(false); } };
                ...
                <tbody className={`divide-y divide-slate-100 ${isLoading ? 'opacity-40' : 'opacity-100'} transition-opacity`}>
                  {users.map((user) => (
                    <tr key={user.id} className="hover:bg-slate-50/80 transition-colors group">
                      ...
                      <td className="px-6 py-4"><div className="flex flex-col items-center gap-1.5"><div className="flex items-center gap-2 text-blue-600 font-bold font-mono"><Wallet className="w-3.5 h-3.5" /> {user.balance || 0}</div><Button variant="outline" size="sm" onClick={() => { setAdjustType('balance'); setAdjustData({ userId: user.id, type: 'add', amount: 0, remark: '' }); setIsAdjustOpen(true); }} className="h-6 px-2 text-[9px] text-blue-600 border-blue-100 hover:bg-blue-50"><Plus className="w-2.5 h-2.5 mr-1" /> 余额调整</Button></div></td>
