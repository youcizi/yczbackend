...
import { pMemberProfiles, pMemberAddresses, pMemberTiers, pMemberTiersI18n } from '../schema/membership';
...
export class MembershipService {
  ...
  static initPlugin() {
    // 订阅定价计算钩子
    hookManager.on('pricing:calculate', async (ctx, data) => {
      ...
      if (discount < 100) {
        ...
        return {
          ...data,
          discountAmount: data.basePrice - finalPrice,
    ...
    console.log('✅ [Membership] Hook "pricing:calculate" 已就绪');
  }
  ...
  static async getTiers(db: any, tenantId: number, locale: string = 'en-US') {
    ...
    .then((rows: any[]) => rows.map(r => ({
      ...r,
      displayName: r.translatedName || r.name // 优先显示翻译
    })));
  }
